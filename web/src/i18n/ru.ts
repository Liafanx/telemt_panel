...
export const ru = {
  ...
  people: {
    ...
    bulkQuota: {
      ...
      menu:"Действия со списком", action:"Сбросить расход квот всем", title:"Сброс расхода квот", all:"Все пользователи · {count}", scope:"Включая отключённые учётные записи и пользователей без лимита. Поиск и фильтры не ограничивают действие.",
      change:"Расход квоты каждого пользователя", preserved:"Останется без изменений", preservedNote:"Лимиты и сроки доступа · учтённый трафик и графики панели · история IP · ссылки и состояние блокировки", effect:"Пользователи с исчерпанной квотой смогут подключаться, если нет других ограничений. Новый расход может начать расти сразу.", agree:"Подтверждаю сброс для всех пользователей ({count}). Отменить выполненный сброс нельзя.", start:"Сбросить всем", refresh:"Загрузить список заново", checking:"Проверяем пользователей и ревизию конфигурации…",
      running:"Сброс выполняется", stopped:"Операция остановлена", partial:"Завершено с отказами", completed:"Расход квот сброшен", progressNote:"Можно закрыть окно и продолжить работу. Прогресс сохранится над списком.", resultNote:"Лимиты, история и параметры доступа сохранены.", processed:"из {count} обработано", confirmed:"Подтверждено", rejected:"Отказано", unknown:"Неизвестно", waiting:"Ожидают", notSent:"Не отправлены", check:"Открыть результат", collapse:"Свернуть", done:"Готово", review:"Требует проверки", exceptionPage:"Записи {start}–{end} из {total}", previous:"Назад", next:"Далее",
      unknownNote:"Не повторяем запрос автоматически: Telemt мог уже сбросить квоту, в том числе перед ошибкой записи на диск. Сначала проверьте состояние пользователя.", remainingNote:"Не отправлено запросов: {count}. Выполненные сбросы не откатываются.", noRetry:"Повторный запуск затронет всех заново. Для отдельных пользователей используйте их страницы.", current:"Проверить текущую операцию", lost:"Состояние операции недоступно или устарело. Не считайте это отменой сброса и не повторяйте его автоматически.", paused:"Связь с панелью потеряна. Показан последний известный прогресс; операция могла продолжиться.", openUser:"Открыть пользователя", serverStopped:"Панель остановлена", expiry:"Подтверждение действительно две минуты. При изменении конфигурации понадобится новое.",
    },
    workspace: {
    ...
    share: {
      ...
      unavailableNoLink: "У пользователя нет корректной ссылки подключения, чтобы построить подписку.",
  ...
  pulse: {
    ...
    problems: {
      ...
      meRuntimeDraining: "ME: соединения выводятся из пула",
      meRuntimeLatency: "ME: выросла задержка соединений",
    ...
    mePool: {
      ...
      explanation: {
        draining: "Telemt завершает старые соединения при замене или перестройке пула. Сам по себе drain не означает нехватку покрытия DC.",
        degradedWriters: "Сглаженный RTT соединений Telemt → ME превысил их адаптивную базу более чем вдвое. Это не фиксированный порог в мс и не потерянные маршруты.",
        coverage: "Доступных соединений с ME меньше требуемого количества. Состояние каждого направления RPC/Media смотрите в дата-центрах.",
      },
      // Строка причины — появляется, только когда карточка не Healthy (§17).
      reason: {
        fallback: "Трафик идёт напрямую, минуя ME",
        coverage: "Покрытие {pct} %",
        draining: "Писателей в дренаже: {count}",
        degradedWriters: "Писателей с повышенным RTT: {count}",
  ...
  server: {
    ...
    settings: {
      ...
      transport: {
        ...
        endpoints: {
          subscriptionTitle: "Страница подписки",
          subscriptionNote: "Отдельный порт только для клиентских страниц. Вход администратора и API панели через него недоступны.",
          disabled: "Отключена",
          enable: "Включить страницу подписки",
          publicURL: "Внешний адрес",
          proxyURL: "Для подписки через reverse proxy укажите внешний адрес с https://. Для доступа без TLS выберите обычный HTTP.",
          path: "Префикс пути",
          addressNote: "Можно вставить полный URL — путь перенесётся в отдельное поле. Прокси должен передавать префикс без изменений. Эти поля не настраивают DNS, nginx/Caddy или firewall. Изменение пути подписки делает старые адреса недействительными.",
          invalidURL: "Укажите полный HTTP(S)-адрес без логина, пароля, параметров и фрагмента.",
          pathRequired: "Укажите путь подписки, например /sub или собственный случайный префикс.",
          httpWarning: "Разрешаю HTTP без шифрования. При публичном доступе персональная ссылка и параметры подключения могут быть перехвачены.",
        },
        ...
        manualDescription: "Для этого запуска недоступна безопасная запись конфигурации. Текущий адрес остаётся активным.",
  ...
  errors: {
    ...
    quota_reset_busy: "Уже выполняется операция с квотами. Дождитесь её результата.",
    quota_confirmation_expired: "Подтверждение истекло или заменено. Загрузите список заново и подтвердите его.",
    quota_operation_unavailable: "Состояние операции недоступно: панель могла перезапуститься. Уже выполненные сбросы не отменены; не повторяйте их автоматически.",
    invalid_quota_schedule: "Проверьте интервал, дату, cron-выражение и часовой пояс: не удалось рассчитать будущие запуски.",
  ...
  auditActions: {
    ...
    "quota.reset_all.started": "Запущен сброс расхода квот всех пользователей",
    "quota.schedule.global": "Изменено общее расписание сброса квот",
    "quota.schedule.user": "Изменено расписание сброса квоты пользователя",
    "quota.schedule.started": "Запущен сброс квот по расписанию",
    "quota.schedule.completed": "Сброс квот по расписанию выполнен",
    "quota.schedule.partial": "Сброс по расписанию завершён с отказами",
    "quota.schedule.stopped": "Сброс по расписанию остановлен",
    "quota.schedule.interrupted": "Сброс по расписанию прерван: проверьте результат",
    "quota.reset_all.completed": "Расход квот всех пользователей сброшен",
    "quota.reset_all.partial": "Массовый сброс завершён с отказами",
    "quota.reset_all.stopped": "Массовый сброс остановлен",
    "quota.reset_all.interrupted": "Массовый сброс прерван: проверьте результат",
