...
- **Пользователи** — создание и редактирование пользователей, квоты и ограничения,
  ссылки подключения, WEB-профили, накопленный трафик и история IP.
  Есть [массовый и плановый сброс расхода квот](docs/QUOTA_RESET.md): подтверждение,
  прогресс, общее расписание и индивидуальная вкладка «Автосброс».
...
- **Оформление** — своё название и логотип из файла на сервере либо вход без
  логотипа. Настраивается для всей панели; [подробнее](docs/BRANDING.md).
...
<details>
<summary>Пользователи, Пульс и настройки — скриншоты десктопа</summary>
...
Вход можно отключить только через `[auth] disabled = true` с перезапуском,
если доступ уже защищён сетью или reverse proxy (например, mTLS).
Это открывает полный доступ всем, кто достиг порта панели; условия и настройка
домена описаны в [конфигурации](docs/CONFIG.md#auth--вход-администратора).
...
- Для установки — root либо sudo. Установщик поддерживает systemd, OpenRC,
  procd/OpenWrt, SysVinit и раскладку Entware под `/opt`. Docker-управление доступно при ручной настройке
  окружения и прав, но установщик не создаёт контейнеры.
- Для нестандартных служб доступны [пользовательские команды и Entware](docs/ENTWARE.md).
...
Тестовый образ — `ghcr.io/amirotin/telemt_panel:1.0.0-rc.2` (amd64/arm64).
...
В режиме reverse proxy дополнительно выбирается путь: корень домена (по умолчанию),
свой или случайный. Установщик записывает `base_path` и показывает примеры nginx/Caddy,
но не меняет конфигурацию прокси. Для установки с `--yes` можно передать
`TP_TLS_MODE=proxy TP_BASE_PATH=/panel` или `TP_BASE_PATH=random`.
...
- [Выбор версии и обновление через панель](docs/UPDATES.md)
- [Запуск, остановка и перезапуск Telemt](docs/SERVICE_CONTROL.md)
...
- [Выбор адреса подключения в ссылке](docs/CONNECTION_LINKS.md)
